...
public class GameLevel implements Animation {
     ...
     * @param runner - as an AnumationRunner object
     * @param score - as a counter that counts the score
     * @param lives - as a countet that counts the lives
    ...
    public void createBlocks() {
        List<Block> douplicate = new ArrayList<>();
        for (int i = 0; i < levelInformation.blocks().size(); i++) {
            douplicate.add(levelInformation.blocks().get(i).copyBlock());
        }

        for (Block b : douplicate) {
    ...
    public void createPaddle(biuoop.KeyboardSensor keyB) {
        ...
        Paddle paddleP = new Paddle(keyB,
                ...
                Color.BLACK, new Point(this.width / 30, 0), new Point(this.width * 29 / 30, 600),
    ...
    public void createBalls() {
        ...
        List<Velocity> douplicateVelocities = new ArrayList<>();
        for (int j = 0; j < listOfVelocity.size(); j++) {
            douplicateVelocities.add(this.levelInformation.initialBallVelocities().get(j).copyVelocity());
        }
        ...
        for (int i = 0; i < numberOfBalls; i++) {
            listOfBalls.add(new Ball(this.width / 2, (int) (this.hight * 0.95 - 15), 6, Color.BLACK));
            listOfBalls.get(i).setVelocity(douplicateVelocities.get(i));
